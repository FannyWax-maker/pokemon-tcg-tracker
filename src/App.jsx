...
export default function App() {
  ...
  const filterCounts = useMemo(() => {
    ...
    pokemonData.forEach(p => {
      if (!p.gen) return;
      // Normalise: handle both numeric gen (new) and string "Kanto (Gen 1)" (old)
      let genNum;
      if (typeof p.gen === 'number') {
        genNum = p.gen;
      } else {
        const m = String(p.gen).match(/\(Gen (\d+)\)/);
        if (!m) return;
        genNum = parseInt(m[1]);
      }
      const pCards = p.cards.filter(c => !c.isSecondary && c.isPrimary !== false);
      if (!genStats[genNum]) genStats[genNum] = { total: 0, owned: 0 };
      genStats[genNum].total += pCards.length;
      genStats[genNum].owned += pCards.filter(c => c.ownedLang).length;
    });
    const setCounts = {};
  ...
  // Derive generations dynamically — handles old format "Kanto (Gen 1)" and new format gen:1, region:"Kanto"
  const generations = useMemo(() => {
    const map = {};
    pokemonData.forEach(p => {
      if (!p.gen) return;
      if (typeof p.gen === 'number') {
        if (!map[p.gen]) map[p.gen] = p.region || `Gen ${p.gen}`;
      } else if (typeof p.gen === 'string') {
        const numMatch = p.gen.match(/\(Gen (\d+)\)/);
        const regionMatch = p.gen.match(/^([^(]+)/);
        if (numMatch) {
          const num = parseInt(numMatch[1]);
          if (!map[num]) map[num] = regionMatch ? regionMatch[1].trim() : `Gen ${num}`;
        }
      }
    });
    return Object.entries(map).map(([g, region]) => [parseInt(g), region]).sort((a, b) => a[0] - b[0]);
  }, [pokemonData]);
  ...
  const filteredData = useMemo(() => {
    ...
    if (filterGeneration !== 'all') {
      ...
      filtered = filtered.filter(p => {
        if (!p.gen) return false;
        if (typeof p.gen === 'number') return p.gen === gen;
        const m = String(p.gen).match(/\(Gen (\d+)\)/);
        return m ? parseInt(m[1]) === gen : false;
      });
    }
              ...
              <div className="pt-3 grid grid-cols-2 md:grid-cols-3 gap-2">
                <select value={filterGeneration} onChange={(e) => setFilterGeneration(e.target.value)} className="px-3 py-1.5 border border-gray-200 rounded-lg text-xs focus:outline-none focus:ring-2 focus:ring-emerald-500">
                  ...
                  {generations.map(([g, region]) => {
                    ...
                    return <option key={g} value={String(g)}>{complete ? '★ ' : ''}Gen {g} · {region} ({s.owned}/{s.total}{!complete ? ` · ${pct}%` : ''})</option>;
                  })}
                </select>
