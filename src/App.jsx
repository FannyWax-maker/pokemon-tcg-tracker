...
export default function App() {
  ...
  return (
    <div className={`min-h-screen font-sans ${darkMode ? 'bg-gray-900' : 'bg-gray-50'}`} style={{fontFamily: "'Segoe UI', system-ui, sans-serif"}}>
      ...
      <div className={`sticky top-0 z-30 ${darkMode ? 'bg-gray-900 border-b border-gray-700' : 'bg-white border-b border-gray-200'}`} style={{boxShadow: darkMode ? 'none' : '0 2px 12px rgba(0,0,0,0.07)'}}>
        <div className="max-w-7xl mx-auto px-3 pt-2 pb-1.5">
          <div className="flex items-center gap-2 flex-wrap">
            ...
            {/* Grid/Cards/Review — hidden on mobile (in bottom nav instead) */}
            <div className={`hidden sm:flex items-center rounded-full p-0.5 text-xs font-bold shrink-0 ${darkMode ? 'bg-gray-700' : 'bg-gray-100'}`}>
            ...
            {/* Ownership toggle — hidden on mobile */}
            {viewMode === 'cards' && (
              <button onClick={() => setShowOwnershipButtons(v => !v)}
                className={`hidden sm:flex p-1.5 rounded-lg transition-colors shrink-0 ${showOwnershipButtons ? 'text-white' : darkMode ? 'bg-gray-700 text-gray-400 hover:bg-gray-600' : 'bg-gray-100 text-gray-500 hover:bg-gray-200'}`}
                style={showOwnershipButtons ? {background: 'linear-gradient(135deg, #22c55e, #16a34a)'} : {}}>
                {showOwnershipButtons ? <Unlock className="w-3.5 h-3.5" /> : <Lock className="w-3.5 h-3.5" />}
              </button>
            )}
            {/* Dark mode — hidden on mobile (in bottom nav) */}
            <button onClick={() => setDarkMode(!darkMode)} className={`hidden sm:flex p-1.5 rounded-lg transition-colors shrink-0 ${darkMode ? 'bg-gray-700 text-yellow-300 hover:bg-gray-600' : 'bg-gray-100 text-gray-600 hover:bg-gray-200'}`}>
              {darkMode ? <Sun className="w-3.5 h-3.5" /> : <Moon className="w-3.5 h-3.5" />}
            </button>
            {/* Lock — hidden on mobile (in bottom nav) */}
            <button onClick={() => isUnlocked ? setIsUnlocked(false) : setShowLockModal(true)} title={isUnlocked ? 'Lock collection' : 'Unlock to edit'}
              className={`hidden sm:flex p-1.5 rounded-lg transition-colors shrink-0 ${isUnlocked ? 'bg-emerald-100 text-emerald-600 hover:bg-emerald-200' : darkMode ? 'bg-gray-700 text-gray-400 hover:bg-gray-600' : 'bg-gray-100 text-gray-500 hover:bg-gray-200'}`}>
            ...
            <button onClick={() => setShowFilters(!showFilters)}
              ...
              <span className="hidden sm:inline">{hasActiveFilters ? activeFilterCount : 'Filters'}</span>
              {hasActiveFilters && <span className="sm:hidden">{activeFilterCount}</span>}
            </button>
            {hasActiveFilters && (
          ...
          <div className="sm:hidden mt-1.5 relative">
            ...
            <input type="text" placeholder="Search Pokémon..." value={searchInput}
              ...
              className={`w-full pl-9 pr-3 py-2.5 rounded-full border text-sm focus:outline-none focus:ring-2 focus:ring-emerald-400 ${darkMode ? 'bg-gray-700 border-gray-600 text-white placeholder-gray-400' : 'border-gray-200 text-gray-700 bg-gray-50'}`}
            />
          </div>
        </div>
      ...
      <div className={`sm:hidden fixed bottom-0 left-0 right-0 z-30 border-t ${darkMode ? 'bg-gray-900 border-gray-700' : 'bg-white border-gray-200'}`}
        style={{ paddingBottom: 'env(safe-area-inset-bottom)' }}>
        <div className="flex items-stretch h-14">
          {/* Grid */}
          <button
            onClick={() => { setViewMode('pokemon'); setFilterHideNonConforming('all'); if (['featured_desc','featured_asc','release_desc','release_asc'].includes(sortBy)) setSortBy('default'); }}
            className={`flex-1 flex flex-col items-center justify-center gap-0.5 text-[10px] font-bold transition-colors ${viewMode === 'pokemon' ? 'text-blue-600' : darkMode ? 'text-gray-400' : 'text-gray-500'}`}
          >
          ...
          {/* Cards */}
          <button
            onClick={() => { setViewMode('cards'); setFilterHideNoCards('all'); setFilterHideNonConforming('hide'); }}
            className={`flex-1 flex flex-col items-center justify-center gap-0.5 text-[10px] font-bold transition-colors ${viewMode === 'cards' ? 'text-blue-600' : darkMode ? 'text-gray-400' : 'text-gray-500'}`}
          >
